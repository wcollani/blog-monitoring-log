...
BEAVER_CMD="beaver -t rabbitmq -c /etc/beaver/beaver.conf -C /etc/beaver/conf.d --logfile ${BEAVER_LOG}"
...
do_start() {
    ...
    su - "${BEAVER_USER}" -s '/bin/bash' -c "${BEAVER_CMD} >> /dev/null 2>&1 & echo \$! > ${BEAVER_PID}"
    echo '.'
}
